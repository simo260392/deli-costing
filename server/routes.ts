...
Return ONLY the JSON object, no explanation.`;
  ...
  // POST /api/product-size-variants/sync — hybrid price sync
  // Strategy:
  //   1. Simple products (type=simple, no options): use Flex catalogue price directly — always retail
  //   2. Combo/platter products: use retail order history (exclude wholesale customer group orders)
  //      Flex doesn't expose per-option prices via API, so we use orders — but filter out wholesale.
  app.post("/api/product-size-variants/sync", async (req: any, res: any) => {
    try {
      ...
      // --- Step 1: Catalogue prices for simple products (Flex product.price) ---
      // key: product_uuid → { sku, price, type }
      const productUuids = [...new Set(allVariants.map((v: any) => v.product_uuid).filter(Boolean))];
      const cataloguePriceMap = new Map<string, number>(); // key: sku|attrs
      const simpleProductSkus = new Set<string>(); // SKUs of simple products — use catalogue price
      ...
      // --- Step 2: Retail order prices for combo/platter products ---
      // Only use orders from NON-wholesale customers
      // key: sku|attrs → highest-seen retail price (retail orders charge more than wholesale)
      ...
      while (true) {
        ...
        for (const order of orders) {
          ...
          const customerGroups: any[] = order.customer_groups || order.customer?.customer_groups || [];
          const isWholesale = customerGroups.some((g: any) => g.uuid === WHOLESALE_GROUP_UUID || g.id === WHOLESALE_GROUP_UUID);
          ...
          for (const item of (order.items || [])) {
            ...
            // Keep highest price seen — retail orders charge more than wholesale
            if (!orderPriceMap.has(key) || price > orderPriceMap.get(key)!) {
              orderPriceMap.set(key, price);
            }
          }
        }
      ...
      // --- Step 3: Apply prices to variants ---
      let priceUpdates = 0;
      for (const row of allVariants) {
        const normAttrs = normaliseAttrs(row.attributes_summary || '');
        const key = `${row.sku}|${normAttrs}`;
        // Catalogue price takes priority for simple products; order price for combos
        const price = cataloguePriceMap.get(key) ?? orderPriceMap.get(key);
      ...
      return res.json({
        ...
        synced: priceUpdates,
        priceUpdates,
        simpleProductsFromCatalogue: cataloguePriceMap.size,
...
// ─── CSV Export/Import ───────────────────────────────────────────────────────
  ...
  app.post("/api/prep/compute", asyncRoute(async (req, res) => {
    ...
    function detectItemType(name: string, flexCategory?: string, categoriesJson?: string): 'wrap' | 'sandwich' | 'toastie' | 'breakfast' | 'other' {
      ...
      // Toasties and bagels — no wrap style, paper only
      if (n.includes('toastie') || n.includes('bagel') || n.includes('focaccia')) return 'toastie';
      // Turkish and similar flat breads are sandwiches (Falafel Turkish, Chipotle Turkish etc.)
      // unless they are a breakfast item — handled below
      ...
      if (n.includes('sandwich') || n.includes('panini') || n.includes('reuben') || n.includes('baguette') || n.includes('roll') || n.includes('lungo') || n.includes('sourdough') || n.includes('turkish')) return 'sandwich';
      return 'other';
    }
    ...
    const skus = [...new Set(orders.map((o: any) => o.sku).filter(Boolean))];
    const flexProductIds = [...new Set(orders.map((o: any) => o.flexProductId).filter(Boolean))];

    const [fpBySku, fpById, sizeVariantsBySku, costingsByProductId, allRecipes, allSubRecipes] = await Promise.all([
      skus.length
        ? supabase.from('flex_products').select('*').in('sku', skus).then(r => {
            const m = new Map<string, any>(); for (const p of (r.data || [])) m.set(p.sku, p); return m;
          })
        : Promise.resolve(new Map<string, any>()),
      flexProductIds.length
        ? supabase.from('flex_products').select('*').in('id', flexProductIds).then(r => {
            const m = new Map<number, any>(); for (const p of (r.data || [])) m.set(p.id, p); return m;
          })
      ...
      supabase.from('recipes').select('*').then(r => {
        const m = new Map<number, any>(); for (const rec of (r.data || [])) m.set(rec.id, rec); return m;
      }),
      supabase.from('sub_recipes').select('*').then(r => {
        const m = new Map<number, any>(); for (const sr of (r.data || [])) m.set(sr.id, sr); return m;
      }),
    ]);
    ...
    for (const order of orders) {
      ...
      for (const comp of components) {
        if (comp.type === 'recipe') {
          const recipe = allRecipes.get(comp.id);
          if (!recipe) continue;
          ...
          for (const sr of subRecipesUsed) {
            const subRecipe = allSubRecipes.get(sr.subRecipeId);
            if (!subRecipe) continue;
          ...
          const subRecipe = allSubRecipes.get(comp.id);
          if (!subRecipe) continue;
...
Respond with ONLY the ID number or the word null. Nothing else.`;
  ...
  app.get("/api/delivery-log", asyncRoute(async (req: any, res: any) => {
    const date = (req.query.date as string) || new Date().toISOString().slice(0, 10);
    // Use +08:00 AWST offset — same as /api/flex-orders which is confirmed working
  ...
  app.post('/api/compliance/logs', async (req: any, res: any) => {
    try {
      ...
      const { data: log, error } = await supabase.from('compliance_logs').insert({
        ...
        thaw_weight_kg: body.thaw_weight_kg ?? body.thawWeightKg ?? null,
        thaw_completed_at: body.thaw_completed_at ?? body.thawCompletedAt ?? null,
  ...
  app.put('/api/compliance/logs/:id', async (req: any, res: any) => {
    try {
      ...
      const fieldMap: Record<string, string> = {
        ...
        thaw_weight_kg: 'thaw_weight_kg', thawWeightKg: 'thaw_weight_kg',
        thaw_completed_at: 'thaw_completed_at', thawCompletedAt: 'thaw_completed_at',
        ...
        try {
          ...
          if (parsed.lineItems.length === 0) {
            for (const line of allLines) {
              ...
              if (pm && wc <= 8 && wc >= 2
                && !/^(Total|Sub|GST|Due|Balance|Dear|Please|Received)/i.test(line)) {
                parsed.lineItems.push({ description: desc, quantity: null, unit: null, numBoxes: null, unitPrice: null, lineTotal: parseFloat(pm[1].replace(/,/g,'')) } as any);
              }
            }
          }
  ...
  app.post('/api/compliance/logs/:id/supplier-lines', async (req: any, res: any) => {
    try {
      ...
      const { data: line, error } = await supabase.from('compliance_supplier_lines').insert({
        ...
        item: body.item || '',
        qty: body.qty || '',
        num_boxes: body.num_boxes ?? body.numBoxes ?? null,
        weight_kg: body.weight_kg ?? body.weightKg ?? null,
        ingredient_id: body.ingredient_id ?? body.ingredientId ?? null,
