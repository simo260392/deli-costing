...
const db = new Database(path.join(__dirname, 'deli-costing/deli.db'));
...
console.log('Columns available:', cols.filter(c => ['id','name','best_cost_per_unit'].includes(c)));
...
results.forEach(r => {
  const diff = ((r.newPrice - r.oldPrice) / r.oldPrice * 100).toFixed(1);
  console.log(`  [${r.id}] ${r.name}: $${r.oldPrice?.toFixed(4) || 'N/A'} → $${r.newPrice.toFixed(4)} (${diff}%)`);
});
